...
POST
/auth/logout
Encerra a sessão ativa, invalidando o token ou refresh token.
POST
/auth/refresh
...
PATH
/users/me
Permite a atualização dos dados cadastrais do próprio usuário.
...
GET
/users
Lista usuários cadastrados. Acesso administrativo.
GET
/users/{id}
Consulta os dados de um usuário específico.
PATCH
/users/{id}/role
Altera o perfil de acesso de um usuário. Acesso administrativo.
PATCH
/users/{id}/status
Ativa, inativa ou bloqueia uma conta. Acesso administrativo
...
GET
/stations 
Lista postos, com suporte a filtros geográficos e comerciais. 
GET
/stations/{id} 
Retorna os detalhes de um posto específico. 
POST
/stations 
Cadastra um novo posto. Acesso administrativo. 
PATH
/stations/{id} 
Atualiza informações de um posto. Acesso administrativo. 
DELETE
/stations/{id} 
Remove ou inativa um posto. Acesso administrativo. 
...
Descrição
GET
/stations/{id}/fuel-prices 
Lista os preços de combustíveis de um posto. 
POST
/stations/{id}/fuel-prices 
Registra um novo preço para um posto. Acesso administrativo ou operador autorizado. 
PATH
/stations/{id}/fuel-prices/{priceId} 
Atualiza um preço registrado. 
GET
/fuel-prices/compare 
Retorna postos e preços comparativos conforme localização e filtros. 
...
GET
/stations/{id}/reviews 
Lista as avaliações de um posto. 
POST
/stations/{id}/reviews 
Registra uma avaliação para o posto. 
PATH
/reviews/{id} 
Atualiza uma avaliação criada pelo próprio usuário. 
DELETE
/reviews/{id}
Remove uma avaliação do próprio usuário ou por moderação administrativa. 
...
Retorna a recomendação de combustível e postos mais vantajosos para um veículo e localização. 
...
O usuário acessa a aplicação e informa seu e-mail e senha.
O sistema valida as credenciais cadastradas.
Quando os dados estão corretos, o sistema gera um token de autenticação para a sessão.
O usuário passa a acessar as funcionalidades permitidas conforme seu perfil de acesso.
Ao encerrar a sessão, o token é invalidado e o usuário precisa realizar um novo login para acessar novamente o sistema.
2. Cadastro e gerenciamento de veículo
O usuário autenticado acessa a área de veículos.
Informa os dados do veículo, como nome/apelido, modelo, ano, tipo de combustível e capacidade do tanque.
O sistema valida e registra as informações vinculadas à conta do usuário.
O usuário pode visualizar, editar ou excluir os veículos cadastrados.
O veículo selecionado pode ser utilizado nos cálculos de consumo e comparação de custos de abastecimento.
3. Consulta e comparação de preços de combustíveis
O usuário informa ou confirma sua localização atual, podendo também selecionar uma região desejada.
O sistema consulta os postos cadastrados próximos à localização informada.
Os postos são exibidos com seus respectivos preços, tipos de combustível, distância e avaliações.
O usuário pode aplicar filtros, como tipo de combustível, faixa de preço, distância e avaliação.
O sistema ordena os resultados para facilitar a comparação, por exemplo, exibindo os menores preços ou os postos mais próximos.
Ao selecionar um posto, o usuário visualiza seus detalhes e os preços disponíveis.
4. Localização de postos
O usuário permite o acesso à sua localização ou informa manualmente um endereço, bairro ou cidade.
O sistema identifica a região de busca.
Os postos disponíveis são apresentados em uma lista e/ou mapa.
O usuário seleciona um posto para consultar informações detalhadas, como endereço, distância, preços cadastrados e avaliações.
Caso desejado, o sistema pode direcionar o usuário para um aplicativo de mapas para traçar a rota até o posto selecionado.
5. Registro de abastecimento e cálculo de consumo
O usuário seleciona um veículo previamente cadastrado.
Informa os dados do abastecimento, como data, quilometragem atual, quantidade de litros, valor total pago e combustível utilizado.
O sistema registra o abastecimento no histórico do veículo.
A partir de abastecimentos consecutivos, o sistema calcula o consumo médio do veículo em quilômetros por litro.
O usuário pode consultar o histórico de abastecimentos, o custo médio por quilômetro e a evolução do consumo do veículo.
...
Após consultar ou utilizar um posto, o usuário autenticado pode registrar uma avaliação.
O usuário informa uma nota e, opcionalmente, um comentário sobre sua experiência.
O sistema valida os dados e vincula a avaliação ao posto e ao usuário responsável.
A avaliação passa a compor a nota média exibida para o posto.
Outros usuários podem consultar as avaliações ao visualizar os detalhes do estabelecimento.
7. Administração e moderação do sistema
Um usuário com perfil administrativo acessa as funcionalidades restritas de gerenciamento.
O administrador pode cadastrar, editar ou remover informações de postos e preços, quando aplicável.
Também pode moderar avaliações que apresentem conteúdo inadequado ou inconsistente.
As ações administrativas são protegidas pelo controle de acesso baseado em perfil.
